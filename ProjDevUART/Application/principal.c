...
void Callback(void){GPIOA->ODR ^= GPIO_ODR_ODR0; }void UART_Callback(void) {int received_data;received_data = MyUART_Read(USART1);printf("%c\n",received_data);GPIOA->ODR ^= GPIO_ODR_ODR0;  // Toggle la broche PA0
